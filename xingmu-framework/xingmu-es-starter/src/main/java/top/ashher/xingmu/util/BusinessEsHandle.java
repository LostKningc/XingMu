...
import top.ashher.xingmu.dto.EsDataQueryDto;
import top.ashher.xingmu.dto.EsDocumentMappingDto;
import top.ashher.xingmu.dto.EsGeoPointDto;
import top.ashher.xingmu.dto.EsGeoPointSortDto;
...
public class BusinessEsHandle {
     ...
     * @param indexName 索引名字
     */
    public void deleteData(String indexName, String indexType) {
        if (!esSwitch) {
            return;
        }
        try {
            ...
            String endpoint;
            if (esTypeSwitch) {
                endpoint = "/" + indexName + "/" + indexType + "/_delete_by_query";
            } else {
                endpoint = "/" + indexName + "/_delete_by_query";
            }

            // 构建 DSL: {"query": {"match_all": {}}}
            String jsonString = "{\"query\":{\"match_all\":{}}}";

            Request request = new Request("POST", endpoint);
            request.setJsonEntity(jsonString); // 注意：如果你的依赖版本较老没有setJsonEntity，请用 setEntity(new NStringEntity(jsonString, ContentType.APPLICATION_JSON));

            Response response = restClient.performRequest(request);
            String responseBody = EntityUtils.toString(response.getEntity());
            log.info("deleteData response: {}", responseBody);

        } catch (Exception e) {
            log.error("deleteData error. indexName: {}", indexName, e);
            throw new RuntimeException("Delete data failed", e);
        }
    }
     ...
     * @return PageInfo
     */
    public <T> Page<T> queryPage(String indexName, String indexType, List<EsDataQueryDto> esDataQueryDtoList, Integer pageNo, Integer pageSize, Class<T> clazz) throws IOException {
     ...
     * @return PageInfo
     */
    public <T> Page<T> queryPage(String indexName, String indexType, List<EsDataQueryDto> esDataQueryDtoList, String sortParam, SortOrder sortOrder, Integer pageNo, Integer pageSize, Class<T> clazz) throws IOException {
     ...
     * @return PageInfo
     */
    public <T> Page<T> queryPage(String indexName, String indexType, EsGeoPointDto esGeoPointDto, List<EsDataQueryDto> esDataQueryDtoList, Integer pageNo, Integer pageSize, Class<T> clazz) throws IOException {
     ...
     */
    public <T> Page<T> queryPage(String indexName, String indexType, EsGeoPointDto esGeoPointDto,
        ...
        List<T> list = new ArrayList<>();
        Page<T> pageInfo = new Page<>(pageNo, pageSize);
        if (!esSwitch) {
            return pageInfo;
        }
        SearchSourceBuilder sourceBuilder = getSearchSourceBuilder(esGeoPointDto,esDataQueryDtoList,sortParam,geoPointDtoSortParam,sortOrder);
            ...
            if (Objects.nonNull(paramValue)) {
                if (paramValue instanceof Collection<?> collection) {
                    if (analyse) {
                        ...
                        QueryBuilder builds = QueryBuilders.termsQuery(paramName, collection);
                        boolQuery.must(builds);
                    }
                }else {
                    QueryBuilder builds;
    ...
    public <T> void executeQuery(String indexName, String indexType,List<T> list,Page<T> pageInfo,Class<T> clazz,
